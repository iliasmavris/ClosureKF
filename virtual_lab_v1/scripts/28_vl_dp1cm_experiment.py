"""
28_vl_dp1cm_experiment.py
=========================
Run the full VL pipeline (Phase 3 sweep + Phase 4 discovery) with
d_p = 0.01 m (1 cm) at 3x velocity.

Physics: d_p=1cm is intermediate between 3mm (works) and 3cm (too heavy).
  Shields ratio ~ 1/d, so 1cm has 3.3x less than 3mm but 3x more than 3cm.
  At 1x velocity: Shields ratio is 1/3 of d=3mm, should give detectable excursions.
  k_spring scales as d^3: 0.02 * (10/3)^3 = 0.74 N/m

Outputs:
  virtual_lab_v1/outputs/sweep_v2_dp1cm/         (sweep + oracle)
  virtual_lab_v1/datasets_v2_dp1cm/              (condition datasets)
  virtual_lab_v1/outputs/phase4_discovery_dp1cm/ (discovery results)

Usage:
  python -u virtual_lab_v1/scripts/28_vl_dp1cm_experiment.py
"""
...
#  CONFIG
# ==========================================================================
VELOCITY_SCALE = 1.0   # 1x velocity (original forcing)

# d_p = 0.01 m physics
D_P = 0.01
K_SPRING = 0.74        # 0.02 * (0.01/0.003)^3 ~ 0.74
MU_S_START = 0.15      # intermediate starting guess for 1x velocity; auto-tune adjusts
MU_K_START = 0.09      # ~60% of mu_s
EPS_X = 0.001           # capture radius: ~10% of d_p

import yaml
with open(VL / "configs" / "ball_params_default.yaml") as f:
    BASE_CFG = yaml.safe_load(f)

# Override for d_p=0.01
BASE_CFG['sphere']['d_p'] = D_P
BASE_CFG['restoring']['k_spring'] = K_SPRING
BASE_CFG['friction']['mu_s'] = MU_S_START
BASE_CFG['friction']['mu_k'] = MU_K_START
BASE_CFG['pin']['eps_x'] = EPS_X
...
SEEDS_PER_VARIANT = 2
SEED_BASE = 400   # new offset to avoid overlap with earlier experiments
...
AUTOTUNE = {
    ...
    'target_er_high': 0.40,
    'mu_s_step': 0.03,
    'mu_s_clamp': [0.02, 1.50],
    'max_iter': 20,
}
BASE_MU_S = MU_S_START
...
OUT_SWEEP   = VL / "outputs" / "sweep_v2_dp1cm"
DS_ROOT     = VL / "datasets_v2_dp1cm"
OUT_DISC    = VL / "outputs" / "phase4_discovery_dp1cm"
...
def main():
    t0_all = time.time()
    print("=" * 70)
    print(f"VIRTUAL LAB: d_p = {D_P} m (1 cm) EXPERIMENT")
    print(f"k_spring = {K_SPRING}, mu_s_start = {MU_S_START}")
    print(f"Velocity scale = {VELOCITY_SCALE}x (original forcing)")
    print("=" * 70)
    ...
    # ---- Step 1: Load forcing variants (no scaling) ----
    print("\n" + "=" * 70)
    print("STEP 1: Load forcing variants (1x velocity)")
    print("=" * 70)
    ...
    print(f"d_p={D_P}, k_spring={K_SPRING}, base_mu_s={BASE_MU_S}, "
          f"mu_s_clamp={AUTOTUNE['mu_s_clamp']}")
    ...
    for i, cond in enumerate(conditions):
        ...
        print(f"  Dataset: ER={er:.3f}, x_std={x_std:.6f} m ({x_std*1000:.3f} mm), "
    ...
    df_summary.to_csv(OUT_SWEEP / "sweep_summary_dp1cm.csv", index=False)
    ...
    df_oracle.to_csv(OUT_SWEEP / "oracle_summary_dp1cm.csv", index=False)
    ...
    # Check oracle gains to decide whether to proceed to discovery
    gains = [r['oracle_gain'] for r in summary_rows
             if r.get('oracle_gain') is not None]
    pos_gains = [g for g in gains if g > 0]
    print(f"Oracle gains: {len(pos_gains)}/{len(gains)} positive")
    if gains:
        print(f"  median={np.median(gains):.3f}, max={max(gains):.3f}")

    if len(pos_gains) < 3:
        print("\nWARNING: Very few positive oracle gains.")
        print("Closure signal may be too weak for discovery.")
        print("Proceeding with Phase 4 anyway for completeness.\n")
    ...
    for cond_dir in cond_dirs:
        ...
        tmp_dir = Path(tempfile.mkdtemp(prefix=f"disc1cm_{cond_id}_"))
        try:
    ...
    df_disc.to_csv(OUT_DISC / "discovery_summary_dp1cm.csv", index=False)

    # ---- Final summary ----
    ok_results = [r for r in all_disc_results if r.get('status') == 'ok']
    print("\n" + "=" * 70)
    print(f"FINAL SUMMARY: d_p = {D_P} m (1 cm) EXPERIMENT")
    print("=" * 70)
